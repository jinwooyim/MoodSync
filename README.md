...
| 기능명 | 설명 | 기여도 |
|--------|------|--------|
| **감정 슬라이더/이미지 프로세싱을 통한 감정 입력** | GitHub Face-API를 이용한 얼굴인식 / Slider를 이용한 6개 감정 조절 | 중 ★ |
| **활동, 도서, 음악 추천 (감정별 100개씩)** | tensorflow.js를 활용한 ML 기반 추천시스템 구현 | 중 ★ |
| **사용자 추천 컬렉션 기능** | 추천받은 항목들을 그룹화 할 수 있는 컬렉션 CRUD 구현 | 하 ☆ |
| **사용자 추천 내기록 기능** | 추천받은 이력 View 및 일자별 감정차트 구현 | 하 ☆ |
| **관리자페이지 구현** | 문의하기, 피드백을 통한 소통공간 창출 및 학습갱신 구현 | 하 ☆ |
| **사용자 문의하기 시계열 분석** | 일자별/시간대별 문의하기 분석차트 구현 | 상 ⭐ |
| **감정 기반 사용자 군집화 (Clustering)** | 일자별 전체 유저의 감정지수 군집화 및 사이트 테마감정 제공 | 상 ⭐ |
| **사용자 이탈 가능성 예측 (Churn Prediction)** | 이탈지표를 이용한 웹사이트 이탈 가능성 예측 | 상 ⭐ |
