  ...
  return (
    <div className="text-center max-w-md mx-auto mb-8"> {/* 외부 div 클래스 유지 */}
      <Card className="p-6">
        ...
        {mappedMoods && (
          <div className="w-full mt-4"> {/* 여백을 위해 mt-4 추가 */}
            {!showResult ? (
              <div
                className="text-center cursor-pointer select-none p-4 border border-purple-200 rounded-lg bg-purple-50 hover:shadow-lg transition"
                onClick={() => setShowResult(true)}
              >
                <p className="text-lg font-semibold mb-2 text-gray-800">감지된 무드 (클릭해서 자세히 보기)</p>
              </div>
            ) : (
