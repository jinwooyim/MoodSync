    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>사용자 문의를 확인하고 답변을 작성할 수 있습니다.</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            <div className="flex items-center justify-center py-8">
              <Loader2 className="w-6 h-6 animate-spin" />
              <span className="ml-2">문의 목록을 불러오는 중...</span>
            </div>
          ) : contacts.length === 0 ? (
                ...
                <div className="space-y-3">
                  {contacts.map((contact) => (
                    <div
                      ...
                      className="py-2 px-4 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer transition-colors"
                    >
      ...
      <Dialog open={!!selectedContact} onOpenChange={() => setSelectedContact(null)}>
        <DialogContent className="sm:max-w-2xl">
          ...
          {selectedContact && (
            <div className="space-y-4">
              <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <h3 className="font-medium mb-2">{selectedContact.contactTitle}</h3>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setSelectedContact(null)} disabled={isReplying}>
              취소
            </Button>
            <Button onClick={handleReplySubmit} disabled={isReplying || !reply.trim()}>
              {isReplying ? (
                <>
